...
## 8. Arquitetura do Sensor de Vazão (Pulsos Brutos e MUX)

O projeto emprega uma abordagem otimizada para medição de vazão de irrigação, compartilhando **um único sensor físico** para aferir a água de múltiplos canteiros.

### 8.1 Ligação Física e Interrupção
Existe apenas um sensor de vazão físico (YF-201) conectado à linha principal de irrigação. Ele está ligado ao **Pino Digital 2 (D2)** do Arduino, escolhido estrategicamente por possuir suporte nativo a interrupção de hardware (`attachInterrupt`).
* A cada volta da hélice (borda de descida / `FALLING`), a interrupção é acionada instantaneamente em segundo plano incrementando o contador.

### 8.2 A Coleta a cada 2 Segundos
No loop principal, a função `updateFlows()` processa a leitura:
* A cada **2 segundos (2000ms)**, ela captura a quantidade de pulsos contada pela interrupção e zera o contador. 
* **Importante:** O nó transmite a quantidade bruta de pulsos acumulada neste pequeno intervalo, e **não** a vazão em litros ou pulsos/minuto. A matemática de conversão deve ocorrer no servidor (Node-RED).

### 8.3 MUX e Compartilhamento da Leitura
Graças à restrição de concorrência imposta ao MUX (apenas um relé atracado por vez), o software consegue deduzir o destino da água:
* Se qualquer válvula de canteiro (Canais 0, 1 ou 2 do MUX) estiver aberta, a função `getFlowForCanteiro()` atribui os pulsos do sensor principal para a medição atual.
* Para o controlador central, todos os pulsos são sempre reportados pelo `CHILD_ID_FLOW_A` (apresentado como `"Vazao.CanteiroA"`). Na prática, esse sensor virtual age como a "Vazão Geral da Linha de Irrigação", independentemente de qual canteiro pediu água.

### 8.4 Formato e Envio da Mensagem
O envio da mensagem é reativo e em **tempo real**: ocorre imediatamente assim que a contagem de pulsos muda em pelo menos 1 pulso (ou retorna a zero). O disparo é feito enviando o número de pulsos como um inteiro.

* **Mensagem Bruta MySensors (Serial):** `99;21;1;0;24;15`
  * `99` (Node ID), `21` (Child ID - Vazão A), `1` (Command: Set), `0` (Ack), `24` (Type: `V_VAR1`), `15` (Payload: pulsos).
* **Tradução no Gateway (MQTT):** O servidor Node-RED recebe o dado no formato MQTT, por exemplo no tópico `m360/out/99/21/1/0/24`, contendo apenas o número `15` como payload.
