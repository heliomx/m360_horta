...
String generateIndexHTML(const DeviceConfig &cfg) {
  ...
  html += "<p><strong>Tópico OUT:</strong> " + buildTopicOut(cfg) + "</p>";
  html += "<p><strong>Tópico IN:</strong> " + buildTopicIn(cfg) + "</p>";
  html += "</div>";
  
  html += "<form action='/save' method='POST'>";
  html += "<label>SSID da Rede WiFi:</label>";
  
  // Usar scan assíncrono: NÃO chamar WiFi.scanNetworks() aqui.
  // WiFi.scanNetworks() síncrono derruba o AP no ESP8266 e corrompe a
  // autenticação do softAP — a senha para de funcionar na 2ª sessão.
  // Usamos o resultado de um scan iniciado anteriormente (handleRoot dispara
  // WiFi.scanNetworks(true) antes de gerar o HTML).
  int n = WiFi.scanComplete();

  html += "<select name='ssid' required>";
  
  if (n == WIFI_SCAN_RUNNING) {
    // Scan ainda em andamento — mostrar campo de texto como fallback
    ...
    // Liberar memória do scan após usar
    WiFi.scanDelete();
    // Inicia novo scan async para a próxima visita à página
...
void handleRoot(DeviceConfig &cfg, ESP8266WebServer &server) {
  // Iniciar scan assíncrono se não houver um em andamento.
  // O resultado será consumido por generateIndexHTML via WiFi.scanComplete().
  // NÃO usar WiFi.scanNetworks() síncrono aqui — ele derruba o AP.
...
void handleSave(DeviceConfig &cfg, ESP8266WebServer &server) {
  ...
  // Debug dos valores recebidos do formulário
