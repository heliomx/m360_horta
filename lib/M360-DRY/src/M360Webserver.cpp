/*
 * M360Webserver.cpp — Portal web de configuração do Gateway M360
 */
...
void handleRoot(M360::M360DeviceConfig &cfg, ESP8266WebServer &server) {
    ...
    server.send(200, "text/html", generateIndexHTML(cfg));
}
...
void handleSave(M360::M360DeviceConfig &cfg, ESP8266WebServer &server) {
    ...
    strncpy(cfg.ssid,     ssidStr.c_str(),       sizeof(cfg.ssid)     - 1); cfg.ssid[sizeof(cfg.ssid)-1]         = '\0';
    strncpy(cfg.mqttServer, mqttServerStr.c_str(), sizeof(cfg.mqttServer) - 1); cfg.mqttServer[sizeof(cfg.mqttServer)-1] = '\0';
    cfg.mqttPort = (uint16_t)port;
    strncpy(cfg.mqttUser, mqttUserStr.c_str(),   sizeof(cfg.mqttUser) - 1); cfg.mqttUser[sizeof(cfg.mqttUser)-1] = '\0';
    strncpy(cfg.uf,       ufStr.c_str(),         sizeof(cfg.uf)       - 1); cfg.uf[sizeof(cfg.uf)-1]             = '\0';
    strncpy(cfg.carNumber, carNumberStr.c_str(),  sizeof(cfg.carNumber) - 1); cfg.carNumber[sizeof(cfg.carNumber)-1] = '\0';
